...
try:
  ...
  st.error(
      f"Gagal memuat file Excel. Pastikan file '{EXCEL_FILE}' ada di direktori"
      f" yang sama. Error: {e}"
  )
  st.stop()
...
if menu == "Laporan Sortir Harian":
  ...
  if not sortir_sheets:
    ...
    try:
      st.info(
          "Berikut adalah data laporan harian yang terbaca dari sheet"
          " terpilih:"
      )
  ...
  search_query = st.text_input(
      "🔍 Cari Data dalam tabel:", "", placeholder="Ketik kata kunci..."
  )
